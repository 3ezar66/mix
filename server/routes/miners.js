...
export default async function (fastify) {
    ...
    fastify.get('/api/miners', {
        ...
        try {
            ...
            const verifiedMiners = await Promise.all(suspectedMiners.map(async (device) => {
                ...
                const newScore = calculateConfidenceScore(device, networkData, rfData, geoData);
                // به‌روزرسانی دیتابیس
...
async function getAllMiners(request, reply) {
    try {
...
function calculateConfidenceScore(device, networkData, rfData, geoData) {
    let score = 0;
    const weights = {
        network: 0.4,
        rf: 0.3,
    ...
    if (networkData) {
        score += weights.network * (networkData.miningPortsDetected * 20 +
            networkData.highBandwidthUsage * 15 +
            networkData.suspiciousConnections * 10);
    }
    // امتیاز بر اساس سیگنال‌های RF
    if (rfData) {
        score += weights.rf * (rfData.powerConsumption * 25 +
