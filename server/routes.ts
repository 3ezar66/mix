...
export async function registerRoutes(app: express.Application): Promise<Server> {
  ...
  app.get("/api/miners", async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/miners/ilam", async (req: Request, res: Response) => {
    try {
      const bounds = {
        north: 34.5,
        south: 32.0,
  ...
  app.post("/api/miners", async (req: Request, res: Response) => {
    try {
      // Remove duplicate declaration since validatedData is already declared below
  ...
  app.patch("/api/miners/:id", async (req: Request, res: Response) => {
    try {
  ...
  app.post("/api/scan/comprehensive", async (req: Request, res: Response) => {
    try {
            ...
            for (const device of results.detected_devices || []) {
              if (device.detection_results?.is_miner) {
                ...
                const minerData = {
                  ...
                  suspicion_score: device.detection_results.confidence_score || 50,
...
`]);
  ...
  app.get("/api/rf-detections", async (req: Request, res: Response) => {
    try {
